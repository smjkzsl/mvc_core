...
namespace Workman.DataBase.SqlServer
{
    ...
    public class Database : IDatabase
    {
        ...
        public int ExecuteBySql(string strSql)
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Execute(strSql, null, dbTransaction);
            }
            catch (Exception)
            {
        ...
        public int ExecuteBySql(string strSql, object dbParameter)
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Execute(strSql, dbParameter, dbTransaction);
            }
            catch (Exception)
            {
        ...
        public int ExecuteByProc(string procName)
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Execute(procName, null, dbTransaction, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public int ExecuteByProc(string procName, object dbParameter)
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Execute(procName, dbParameter, dbTransaction, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public T ExecuteByProc<T>(string procName) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().ExecuteScalar<T>(procName, null, dbTransaction, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public T ExecuteByProc<T>(string procName, object dbParameter) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().ExecuteScalar<T>(procName, dbParameter, dbTransaction, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public IEnumerable<T> QueryByProc<T>(string procName) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Query<T>(procName, null, dbTransaction, true, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public IEnumerable<T> QueryByProc<T>(string procName, object dbParameter) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Query<T>(procName, dbParameter, dbTransaction, true, null, CommandType.StoredProcedure);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTableByProc(string procName, object dbParameter)
        {
            try
            {
                var IDataReader = dbcontext.Database.GetDbConnection().ExecuteReader(procName, dbParameter, dbTransaction, null, CommandType.StoredProcedure);
        ...
        private void UpdateEntity<T>(T entity) where T : class
        {
            ...
            foreach (string item in props.Keys)
            {
                ...
                if (value != null)
                {
                    ...
                    //主键不能被更新
                    if (!dbcontext.Entry(entity).Property(item).Metadata.IsPrimaryKey())
                    {
        ...
        public T FindEntity<T>(string strSql, object dbParameter = null) where T : class, new()
        {
            try
            {
                var data = dbcontext.Database.GetDbConnection().Query<T>(strSql, dbParameter, dbTransaction);
        ...
        public IEnumerable<T> FindList<T>(string strSql) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Query<T>(strSql, null, dbTransaction);
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<T> FindList<T>(string strSql, object dbParameter) where T : class
        {
            try
            {
                return dbcontext.Database.GetDbConnection().Query<T>(strSql, dbParameter, dbTransaction);
            }
            catch (Exception)
            {
        ...
        public IEnumerable<T> FindList<T>(string strSql, object dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class
        {
            try
            {
                ...
                return dbcontext.Database.GetDbConnection().Query<T>(sb.ToString(), dbParameter, dbTransaction);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTable(string strSql)
        {
            try
            {
                var IDataReader = dbcontext.Database.GetDbConnection().ExecuteReader(strSql, null, dbTransaction);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
            catch (Exception)
            {
        ...
        public DataTable FindTable(string strSql, object dbParameter)
        {
            try
            {
                var IDataReader = dbcontext.Database.GetDbConnection().ExecuteReader(strSql, dbParameter, dbTransaction);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
        ...
        public DataTable FindTable(string strSql, object dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            try
            {
                ...
                sb.Append(SqlHelper.SqlPageSql(strSql, orderField, isAsc, pageSize, pageIndex));
                total = Convert.ToInt32(dbcontext.Database.GetDbConnection().ExecuteScalar("Select Count(1) From (" + strSql + ") As t", dbParameter));
                var IDataReader = dbcontext.Database.GetDbConnection().ExecuteReader(sb.ToString(), dbParameter, dbTransaction);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
            }
            catch (Exception)
            {
        ...
        public object FindObject(string strSql, object dbParameter)
        {
            try
            {
                return dbcontext.Database.GetDbConnection().ExecuteScalar(strSql, dbParameter, dbTransaction);
            }
            catch (Exception)
            {
            ...
            try
            {
                ...
                strSql.Append(@"DECLARE @TableInfo TABLE ( name VARCHAR(50) , sumrows VARCHAR(11) , reserved VARCHAR(50) , data VARCHAR(50) , index_size VARCHAR(50) , unused VARCHAR(50) , pk VARCHAR(50) )
                            ...
                            FROM @TableInfo F LEFT JOIN ( SELECT name = CASE WHEN A.COLORDER = 1 THEN D.NAME ELSE '' END , tdescription = CASE WHEN A.COLORDER = 1 THEN ISNULL(F.VALUE, '') ELSE '' END FROM SYSCOLUMNS A LEFT JOIN SYSTYPES B ON A.XUSERTYPE = B.XUSERTYPE INNER JOIN SYSOBJECTS D ON A.ID = D.ID AND D.XTYPE = 'U' AND D.NAME <> 'DTPROPERTIES' LEFT JOIN sys.extended_properties F ON D.ID = F.major_id WHERE a.COLORDER = 1 AND F.minor_id = 0 ) P ON F.name = p.name
                             ORDER BY f.name  ");
                return dbcontext.Database.GetDbConnection().Query<T>(strSql.ToString(), null, dbTransaction);
            }
            catch (Exception)
            {
        ...
        public IEnumerable<T> GetDBTableFields<T>(string tableName) where T : class, new()
        {
            try
            {
                ...
                return dbcontext.Database.GetDbConnection().Query<T>(strSql.ToString(), new { tableName = tableName }, dbTransaction);
            }
            catch (Exception)
            {
